...
file_path = '/content/drive/MyDrive/FYP_Project/FYP_Final_Data.csv'
...
er_df = pd.read_csv('/content/drive/MyDrive/FYP_Project/Exchange Rate - Coconut.csv')
er_df.head()
...
fresh_df = pd.read_csv('/content/drive/MyDrive/FYP_Project/Cleaned_Fresh_Nut_Consumption.csv')
fresh_df.head()
...
local_price_df = pd.read_csv('/content/drive/MyDrive/FYP_Project/Coconut_local_price.csv')
local_price_df.head()
...
output_path_preprocessed = '/content/drive/MyDrive/FYP_Project/FYP_Preprocessed_Data.csv'
...
output_path_fe = '/content/drive/MyDrive/FYP_Project/FYP_Feature_Engineered_Data.csv'
...
print(f"\n✓ Data loaded: {df_model.shape[0]} rows, {df_model.shape[1]} columns")
...
if missing_in_X > 0 or missing_in_y > 0:
    print("\n⚠ WARNING: Missing values detected! We'll handle this next.")
else:
    print("\n✓ No missing values - data is ready!")

# Add Date back temporarily for splitting
X['Date'] = df_model['Date']

# Define split points
train_end_date = '2023-12-31'  # Train: 2015-2023
validation_start_date = '2024-01-01'  # Validation: 2024

# Split into train and validation
X_train = X[X['Date'] <= train_end_date].copy()
X_val = X[X['Date'] >= validation_start_date].copy()

y_train = y[X['Date'] <= train_end_date].copy()
y_val = y[X['Date'] >= validation_start_date].copy()

# Remove Date from feature sets (we don't use it as a feature)
...
print(f"  Date range: {df_model.loc[X_train.index, 'Date'].min()} to {df_model.loc[X_train.index, 'Date'].max()}")

print(f"\n✓ Validation Set (2024)")
print(f"  X_val shape: {X_val.shape}")
print(f"  y_val shape: {y_val.shape}")
print(f"  Date range: {df_model.loc[X_val.index, 'Date'].min()} to {df_model.loc[X_val.index, 'Date'].max()}")
...
print(f"\n✓ Split Summary:")
print(f"  Training:   {len(X_train):4d} samples ({train_pct:.1f}%)")
print(f"  Validation: {len(X_val):4d} samples ({val_pct:.1f}%)")
print(f"  Total:      {len(X):4d} samples (100.0%)")

# Verify no data leakage
if X_train.index.max() < X_val.index.min():
    print("\n✓ No temporal overlap - data split is valid!")
else:
...
print(f"  X_train index: 0 to {X_train.index.max()}")
print(f"  X_val index:   0 to {X_val.index.max()}")

print("="*70)
print("DATA SUFFICIENCY ANALYSIS")
print("="*70)

# 1. Check samples per product
print("\n[1] SAMPLES PER PRODUCT (Training Set)")
print("-"*70)

# Reconstruct product names from one-hot encoding
...
val_product_counts = X_val[product_cols].sum().astype(int)
val_product_counts.index = val_product_counts.index.str.replace("Product_", "")
val_product_counts = val_product_counts.sort_values(ascending=False)

# Combine into a summary table
product_summary = pd.DataFrame({
    'Train_Samples': train_product_counts,
    'Val_Samples': val_product_counts
})
product_summary['Total'] = product_summary['Train_Samples'] + product_summary['Val_Samples']
product_summary['Train_%'] = (product_summary['Train_Samples'] / product_summary['Total'] * 100).round(1)

print(product_summary.sort_values('Total', ascending=False))

# 2. Check feature-to-sample ratio
...
if ratio > 10:
    print("  ✓ GOOD: Ratio > 10 - sufficient samples per feature")
elif ratio > 5:
    print("  ⚠ MODERATE: Ratio 5-10 - acceptable but watch for overfitting")
else:
    print("  ✗ POOR: Ratio < 5 - risk of overfitting, consider reducing features")

# 3. Check class balance (Product distribution)
print("\n[3] PRODUCT DISTRIBUTION COMPARISON")
print("-"*70)

train_distribution = (train_product_counts / train_product_counts.sum() * 100).round(2)
val_distribution = (val_product_counts / val_product_counts.sum() * 100).round(2)

dist_comparison = pd.DataFrame({
    'Train_%': train_distribution,
    'Val_%': val_distribution,
    'Difference': (train_distribution - val_distribution).round(2)
})

print(dist_comparison.sort_values('Train_%', ascending=False))

# Flag any large differences
...
if not large_diff.empty:
    ...
    print("\n✓ Product distributions are consistent across train/val")
...
# Check if validation mean is significantly different
mean_diff_pct = abs(y_train.mean() - y_val.mean()) / y_train.mean() * 100
print(f"\n  Mean difference: {mean_diff_pct:.1f}%")

if mean_diff_pct > 30:
    print("  ⚠ Large shift in target distribution (possible market change in 2024)")
    print("  → This is NORMAL for export data - models should still work")
elif mean_diff_pct > 15:
    print("  ⚠ Moderate shift detected - monitor model performance closely")
else:
    print("  ✓ Target distributions are similar")

# 5. Overall Verdict
print("\n" + "="*70)
print("OVERALL DATA SUFFICIENCY VERDICT")
print("="*70)

print(f"""
  Total Training Samples:     {n_train_samples}
  Total Validation Samples:   {len(X_val)}
  Number of Features:         {n_features}
  Products:                   {len(product_cols)}
  Sample-to-Feature Ratio:    {ratio:.1f}x
  Avg Samples per Product:    {n_train_samples / len(product_cols):.0f}

  ✓ Training set is sufficient for:
    • Linear Regression (needs ~30+ samples)
    • Random Forest (needs ~50+ samples)
    • XGBoost (needs ~50+ samples)

  📌 RECOMMENDATION:
    • Your data is SUFFICIENT for all planned models
    • 1284 training samples with 29 features = {ratio:.1f}x ratio
    • Each product has ~{n_train_samples // len(product_cols)} months of history
    • When 2025 data arrives, add it as a separate test set
""")
...
# Columns to exclude
exclude_cols = [
    'Date',           # Already encoded as Month_sin/cos
    'Volume',         # This is our target
    'FOB_Value_USD',  # Calculated FROM Volume (data leakage)
    'Product',        # Already one-hot encoded
    'volume_z',       # Derived from target
    'Cluster',        # Not a predictor
    'Cluster_Name'    # Not a predictor
]

# Select features
feature_cols = [col for col in merged_df_fe.columns if col not in exclude_cols]

# Create X and y
X = merged_df_fe[feature_cols].copy()
y = merged_df_fe[target].copy()

# Time-based split
# Train: 2015-2023, Validation: 2024
train_mask = merged_df_fe['Date'] <= '2023-12-31'
val_mask = merged_df_fe['Date'] >= '2024-01-01'
...
X_val_scaled = pd.DataFrame(
    scaler.transform(X_val),         # Only apply on val
    columns=X_val.columns
)

# Print summary
print(f"\n✓ Features selected: {len(feature_cols)}")
print(f"✓ Training set:   {X_train_scaled.shape[0]} samples (2015-2023)")
print(f"✓ Validation set: {X_val_scaled.shape[0]} samples (2024)")
...
# ─────────────────────────────────────────────
axes[0, 0].scatter(y_val, y_val_pred_lr, alpha=0.6,
                   edgecolors='black', linewidth=0.5, color='steelblue')

# Perfect prediction line (red dashed)
min_val = min(y_val.min(), y_val_pred_lr.min())
max_val = max(y_val.max(), y_val_pred_lr.max())
axes[0, 0].plot([min_val, max_val], [min_val, max_val],
                'r--', linewidth=2, label='Perfect Prediction')

axes[0, 0].set_xlabel('Actual Volume', fontsize=12)
axes[0, 0].set_ylabel('Predicted Volume', fontsize=12)
axes[0, 0].set_title('Actual vs Predicted Volume', fontsize=13, fontweight='bold')
axes[0, 0].legend()
...
axes[0, 1].axvline(residuals.mean(), color='green', linestyle='--',
                   linewidth=2, label=f'Mean: {residuals.mean():.2f}')
axes[0, 1].set_xlabel('Error (Actual - Predicted)', fontsize=12)
...
lr_mape = np.mean(np.abs((y_val - y_val_pred_lr) / y_val)) * 100

# Print results
print(f"\n✓ Model trained and predicted!")
print(f"\n  Performance on 2024 Validation Set:")
print(f"  ─────────────────────────────────────")
...
def smape(actual, predicted):
    ...
    Handles small values better than MAPE
    """
...
lr_rmse = np.sqrt(mean_squared_error(y_val, y_val_pred_lr))
lr_r2 = r2_score(y_val, y_val_pred_lr)
lr_mape_old = np.mean(np.abs((y_val - y_val_pred_lr) / y_val)) * 100
lr_smape = smape(y_val, y_val_pred_lr)
...
print(f"  RMSE:       {lr_rmse:.2f}")
print(f"  R²:         {lr_r2:.4f}")
print(f"  Old MAPE:   {lr_mape_old:.2f}%  ")
